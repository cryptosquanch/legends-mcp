...
export interface AutoMatchInput {
  question: string;
  max_matches?: number; // Default: 2, Max: 3
  include_prompts?: boolean; // Whether to include full system prompts (default: false for lighter response)
}
...
const EXPERTISE_KEYWORDS: Record<string, { legends: string[]; expertise: string }> = {
  // Crypto & Web3
 ...
 */
function findMatches(question: string, maxMatches: number): { legendId: string; reason: string; matchedExpertise: string[] }[] {
  const q = question.toLowerCase();
  const matchScores = new Map<string, { score: number; reasons: string[]; expertise: string[] }>();
 ...
 */
function extractKeyInsight(legend: LegendSkill): string {
 ...
 */
export function autoMatch(input: AutoMatchInput): AutoMatchResponse {
  ...
  // Build response
  const matchedLegends: MatchedLegend[] = matches.map(match => {
    const legend = getLegendById(match.legendId);
    if (!legend) return null;

    const result: MatchedLegend = {
      legend_id: legend.id,
      name: legend.name,
    ...
    return result;
  }).filter((m): m is MatchedLegend => m !== null);
 ...
 */
function generateApproach(question: string, matches: MatchedLegend[]): string {
  if (matches.length === 1) {
 ...
 */
export function formatAutoMatch(response: AutoMatchResponse): string {
  const lines: string[] = [
